...
try {
    $product = new Product(generate_uuid(), $data->isActive, $data->name, $data->type, $data->price, $data->quantity, $data->description,
            $data->material, $data->style, $data->album, $data->model, $data->connect);
    ...
    $db->query("INSERT INTO product(Id, Is_active, Name, Type, Price, Quantity, Description, Material, Style, Album, Model, Connect)
    VALUES ('" .$productData['productId']. "','" .$productData['productIsActive']. "','" .$productData['productName']. "','" .$productData['productType']. "','"
    .$productData['productPrice']. "','" .$productData['productQuantity']. "','" .$productData['productDescription']. "','" .$productData['material']. "','"
    .$productData['style']. "','" .$productData['album']. "','" .$productData['model']. "','" .$productData['connect']. "')");
